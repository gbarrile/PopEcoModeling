...
# Code last updated on 7/20/2021 by Gabe Barrile
...
# we model the effect of the covariate on the log scale, as is customary for the Poisson 
...
# In place of the line of code with logit(), you could also use the following to lines:
#lp[i,j] <- alpha.p + beta1.p * time[i,j]
#p[i,j] <- exp(lp[i,j])/(1+exp(lp[i,j]))
}
}

# we model the effect of the covariate on the logit scale, as is customary for the Binomial 
